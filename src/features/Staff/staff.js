...
const Staff = () => {
  ...
  useEffect(() => {
    getPtDetail(staffInfo["AccountStaff.id"])
      .then((response) => {
            ...
            <div className="PtName">
              ...
              <div className="btnPT">
                {staffInfo["oleId"] === 4 ? (
                  <Link to={`/scanqr`}>
                    {" "}
                    <button className="btnBook">Scan QR</button>
                  </Link>
                ) : (
                  <Link to={`/pt-booking/`}>
                    {" "}
                    <button className="btnBook">Lịch booking</button>
                  </Link>
                )}
                <Link to={`/staff-info/${ptDetail?.id}`}>
                  {" "}
                  <button className="btnInfo">Thông tin cá nhân</button>
                </Link>

                <button className="btnSalary" onClick={openModal}>
                  Salary
                </button>
                <Modal
                  isOpen={modalIsOpen}
