...
import { getAllDiscount, getAllService } from "../serviceAPI";
  ...
  const serverSideDatasource = useCallback(() => {
    ...
    return {
      getRows: function (params) {
        ...
        try {
          //FOR FILTER PURPOSE
          getAllDiscount(props.searchValue, parseInt(page))
            .then((res) => {
              // console.log(res.data.data);
              const data = res.discount;
              if (data && data.rows.length > 0) {
                const lastRow = () => {
                  if (parseInt(data.totalPage) <= 1) return data.count;
                  else if (page <= parseInt(data.totalPage) - 2) return -1;
