...
import { handleGetChartBookingInMonth } from "../../OverviewBranchCenter/CenterAPI";
...
const AreaChart = () => {
  ...
  useEffect(() => {
    handleGetChartBookingInMonth(1).then((res) => {
      const newArr = [];
      ...
      countCreatedAtArr.forEach((el) => {
        ...
        if (
          ...
          if (
            ...
            const newObj = {
              id: el.id,
              createdAt: moment(el.createdAt).format("dddd, DD-MM"),
              count: newCount,
            };
            newArr.push(newObj);
          }
